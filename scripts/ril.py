    ...
    if node["type"] == "decision" and args.field:
        ...
        for pair in args.field:
            if pair.split("=", 1)[0] not in editable:
                fail(
                    "decision nodes are immutable; record a new decision "
                    "and add a supersedes edge instead"
                )
...
def cmd_lock(args: argparse.Namespace) -> None:
    ...
    node["lock_until"] = datetime.now(timezone.utc).isoformat(timespec="seconds").replace(
        "+00:00", "Z"
    )
    node["lock_until"] = (
        datetime.fromisoformat(node["lock_until"].replace("Z", "+00:00"))
    ).isoformat()
...
def cmd_stale(args: argparse.Namespace) -> None:
    ...
    marked = []
    for node in graph["nodes"].values():
        if node["type"] in {"hypothesis", "task"} and node["status"] == "active":
            if node.get("touched_round", 0) < cutoff:
                node["status"] = "stale"
                node["version"] += 1
                node["updated_at"] = now()
...
def cmd_check(args: argparse.Namespace) -> None:
    ...
    for node in graph["nodes"].values():
        if node["id"] not in connected and node["type"] in {
            "issue",
            "hypothesis",
            "task",
            "change",
        }:
            problems.append(f"orphan {node['id']} ({node['type']})")
    ...
    for source in deps:
        if has_cycle(source):
            problems.append(f"dependency cycle involving {source}")
    for node in graph["nodes"].values():
        if node["type"] == "hypothesis" and node["status"] == "active":
            has_evidence = any(
                edge["to"] == node["id"] and edge["type"] in {"validates", "refutes"}
                for edge in graph["edges"]
            )
            if not has_evidence:
