...
status: in_progress
last_updated: "2026-07-22T13:20:36.000Z"
last_activity: 2026-07-22 — v31.0 P40 PagedKvCacheWrapper + T8 doc fix (Phase 31-G closed + CI green; OPS-32a first half shipped — concrete `vllm_dist::BlockDataSource` impl wraps `Arc<PagedKvCache>`, `fetch_block` reads per-layer K/V via new `pub(crate) read_layer_block` helper + serializes via `bytemuck::cast_slice` + dequantizes quantized caches + returns via `tokio::task::block_in_place`; `has_block` queries `block_hashes[layer 0]` as the canonical witness; entire module gated by `#[cfg(feature = "multi-node")]` per ADR-008; 6 new unit tests + 1 2-node gRPC round-trip integration test; `OPERATIONS.md` §"Multi-Node (Experimental)" §"What works" grows from 3 → 4 bullets; ADR-020 status line bumped from "engine integration deferred to v32+" to "wrapper shipped in v31.0 P40; engine plumbing deferred to P41+"; master plan gains Phase Index row **31-G Multi-Node Wrapper**; engine plumbing `Arc<PagedKvCache> → MemoryManager::block_data_source + EngineBuilder::with_paged_kv_cache + server main` deferred to P41+ per the spec's scope-split rationale); T8 doc-fix batch then closed 13 pre-existing broken intra-doc links across 6 files (`vllm-traits/src/sampling.rs` 4 — cross-crate / cross-module `crate::sampling::sample_*` paths that pointed to `vllm_core::sampling` not `vllm_traits::sampling`, plus `vllm_traits::FinishReason::Stop` on the `stop_token_sequences` builder; `vllm-core/src/sampling.rs` 1 — unqualified `[`sample_one_with_params`]` in module-level doc; `vllm-core/src/scheduler/memory/mod.rs` 5 — entire "Multi-node cache coherence" section's intra-doc links to items gated behind `#[cfg(feature = "multi-node")]` so default-features doc build failed; `vllm-server/src/openai/{chat,completions}.rs` 4 — `[`TokenId`]` without `vllm_traits::` prefix + two `[`super::completions::...`]` links to private items in the sibling module; `vllm-model/src/paged_tensor/tensor_store/mod.rs` 1 — `[`crate::paged_kv_cache_wrapper`]` only resolves under `--features multi-node`); all 13 replaced with descriptive backtick-quoted text per project style; zero behavior change, zero public-API delta, zero test delta; `cargo doc --workspace` (both default and `--all-features`) now green; doc-coverage real 67.91% → 68.0%; `just ci` green workspace-wide; engine plumbing (`Arc<PagedKvCache> → MemoryManager::block_data_source + EngineBuilder::with_paged_kv_cache + crates/server/src/main.rs wiring + hash-verification helper on wrapper side + receiver-side `write_kv_batch` integration`) is the natural P41 candidate (OPS-32a second half) — bounded scope (5 reviewable tasks), closes the multi-node story end-to-end; alternatives at this branch point: OTLP exporter (production-readiness §6, needs collector + dep decision, MEDIUM-HIGH ROI), checksums+provenance signed half (engineering-quality §7, needs signature-key story, MEDIUM ROI), MLA RopeScaling (gated on MLA production wiring, LOW ROI), Phase 31-F follow-ups `expand_kv` fused kernel + PagedKV host round-trip elimination (both very-high complexity, deferred until measured)
follow-up batches: |-
