...
last_activity: 2026-07-21 — v0.x wire-type P38 stop sequences engine wire-through on chat + completions + large_enum_variant clippy fix (closes the LAST hard-400 v0.x wire-type carve-out on chat + completions — stop was declared + validated in P22/P32 but engine honoring was deferred because it required tokenization + per-step suffix check that the logprob-generation primitive from P36 unblocked; the v0.x wire-type backlog is now FULLY CLOSED at both layers; 1 new public-API field SamplingParams::stop_token_sequences Option<Vec<Vec<TokenId>>> + 2 new builder methods with_stop_token_sequences / with_stop_token_sequences_none (non-const because Option<Vec<Vec<TokenId>>> is non-Copy) — pre-tokenized at the HTTP boundary via state.tokenizer.encode so the engine and sampler never touch the tokenizer; 1 new private helper vllm_core::sampling::matches_stop_sequences(&[u32], &[Vec<u32>]) -> bool (O(M) per check — single suffix position per stop, NOT O(N × M); slice-eq is SIMD-accelerated; ~32 token comparisons per step); 1 new private helper vllm_server::openai::sampling_validation::validate_stop_sequences(&Option<Vec<String>>) — rejects > 4 strings (OpenAI spec upper bound) and empty/whitespace-only strings (would tokenize to zero tokens); chat handler tokenizes each stop string at HTTP boundary (returns 400 on all-zero-token defensive edge case) and forwards via populate_completion_sampling_params-extended-with-third-parameter pattern (keeps populator pure); completions populator signature extended to accept pre-tokenized stops (tokenize in caller to keep populator pure; best-of path inherits per-candidate stop set automatically); engine's step_regular runs the stop check after sample_batch_with_params and before response_tx send — on match calls finalize_finished(seq_id, FinishReason::Stop) AFTER emitting the matched token via response_tx (OpenAI convention — response includes the matched stop text); old 400-rejection blocks in both validators replaced with validate_stop_sequences calls; old test_chat_rejects_non_empty_stop_with_400 renamed to test_chat_stop_now_accepted_by_handler with new assert_ne!(400) assertion; matched token still carries its logprob via SampledToken (P36 compose); streaming emits finish_reason="stop" on the chunk carrying the matched token + [DONE] sentinel; max_tokens wins if hit before stop (Length), stop wins if matched before max_tokens (Stop); Part C — EngineMessage::AddRequest.request changed from Request to Box<Request> to silence clippy::large_enum_variant (P36's SampledToken inflation pushed field past 248 bytes; other variants ≤ 64 bytes; 8 construction sites updated to Box::new); Engine::add_request public API unchanged (consumption site uses *request deref); 7 pre-existing clippy::uninlined_format_args errors in test files also fixed (masked by early-termination on large_enum_variant); 8 new matches_stop_sequences unit tests in crates/core/src/sampling/tests.rs (single_token / multi_token / partial / first_match_wins / empty_stops / stop_longer_than_generated / empty_generated + the 7 already approved) + 7 new validator tests + 9 new integration tests (chat stop triggers finish_reason_stop / chat multi_token / chat multiple_stops / completions stop / completions best_of_each_candidate_honors_stop / chat logprobs_returns_logprobs_of_stopped / chat streaming_finish_reason_stop_on_last_chunk / completions streaming_finish_reason_stop_on_last_chunk / chat max_tokens_stop_wins_when_earlier) + 1 renamed test; the v0.x wire-type backlog is now FULLY CLOSED at both the declaration + validation layer AND the engine-honoring layer — every chat + completions OpenAI-spec field is end-to-end except tools/tool_choice (grammar-constrained decoder, P33 declared only) and n > 1 (sample-N parallel, P39 candidate); just ci is now green workspace-wide; P38 ship gate met
follow-up batches: |-
