...
| `echo` / `suffix` / `best_of` (legacy `/v1/completions`) | **`echo` + `suffix` shipped in P35 (2026-07-20, engine wire-through); `best_of` remains v32+** — `CompletionRequest::echo` (`Option<bool>`) + `CompletionRequest::suffix` (`Option<String>`, any string per OpenAI spec) + `CompletionRequest::best_of` (`Option<u32>`, range `>= 1`) are all declared + validated end-to-end through the HTTP boundary (P32). `echo` and `suffix` engine honoring closed by P35 via the private `apply_completion_meta` helper in `crates/server/src/openai/completions.rs` — non-streaming prepends prompt + appends suffix directly on `choices[0].text`; streaming puts the prompt on the first non-empty text chunk and the suffix on the finish_reason chunk (matches OpenAI's accumulator semantics). **`best_of` remains v32+** — it requires the engine to sample N times + rank by mean logprob, which depends on the same v32+ logprob-generation work as the `logprobs` field (P31). |
