...
| `echo` / `suffix` / `best_of` (legacy `/v1/completions`) | **`echo` + `suffix` shipped in P35 (2026-07-20, engine wire-through); `best_of` is now in reach** — `CompletionRequest::echo` (`Option<bool>`) + `CompletionRequest::suffix` (`Option<String>`, any string per OpenAI spec) + `CompletionRequest::best_of` (`Option<u32>`, range `>= 1`) are all declared + validated end-to-end through the HTTP boundary (P32). `echo` and `suffix` engine honoring closed by P35 via the private `apply_completion_meta` helper in `crates/server/src/openai/completions.rs` — non-streaming prepends prompt + appends suffix directly on `choices[0].text`; streaming puts the prompt on the first non-empty text chunk and the suffix on the finish_reason chunk (matches OpenAI's accumulator semantics). **`best_of` was the last v0.x carve-out** — it required sample-N-rank-by-mean-logprob, and the logprob-generation primitive that work depended on was just shipped in P36 (`SampledToken::logprob`). The remaining work is the sample-N + rank loop itself, which is mechanical (not a wire-breaking change). Tracked as the next candidate after P36. |
...
- The v0.3 `logprobs` + `top_logprobs` declaration + validation
  ...
  `best_of` (legacy `/v1/completions`) and `tools` /
  `tool_choice` (grammar-constrained decoder) remain.
