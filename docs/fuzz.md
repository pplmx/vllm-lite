...
| `gguf_header`              | `gguf` crate                              | bytes → `GgufHeader::deserialize`                      | v31.0   |
| `tokenizer_decode`         | `tiktoken` + tokenizer                    | bytes → `Tokenizer::decode`                            | v31.0   |
| `openai_http_request`      | `crates/server/src/openai/types.rs`       | bytes → `serde_json::from_slice::<ChatRequest>`        | v31.0   |
| `completions_http_request` | `crates/server/src/openai/types.rs`       | bytes → `serde_json::from_slice::<CompletionRequest>`  | v31.0   |
| `batch_json_input`         | `crates/server/src/openai/batch/types.rs` | bytes → `serde_json::from_slice::<SimpleBatchRequest>` | v31.0   |
...
**30 seconds per target** across the matrix (8 targets). Uploads crash
artifacts on failure. Wall-clock: ~5-7 min total.
...
**5 minutes per target** (40 min wall-clock for 8 targets). Uses a
