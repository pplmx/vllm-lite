...
#
# GOV-01 (technical due diligence): the image version label
# (`org.opencontainers.image.version`) is supplied via the
# `VLLM_VERSION` build arg. release.yml sources
# `target/release-manifest.env` (generated by
# `scripts/release-manifest.sh`) and passes `--build-arg
# VLLM_VERSION=$VLLM_VERSION` so the binary built from the
# same Cargo.lock is tagged with the same version that the
# Helm chart and GitHub Release report.
#
...
# `VLLM_VERSION` is overridden at build time by release.yml. The
# default ("dev") makes `docker build .` work locally without
# needing the release manifest script.
ARG VLLM_VERSION=dev
ARG VLLM_GIT_SHA=unknown
ARG VLLM_BUILD_TIMESTAMP=unknown
...
# OCI image labels. `org.opencontainers.image.version` is the
# canonical place registries (GHCR, Docker Hub) read for the
# displayed version. The values come from the build args
# declared above so `docker build --build-arg VLLM_VERSION=...`
# flows through end-to-end (GOV-01).
LABEL org.opencontainers.image.title="vllm-lite" \
      org.opencontainers.image.description="Lightweight LLM inference engine in Rust" \
      org.opencontainers.image.source="https://github.com/pplmx/vllm-lite" \
      org.opencontainers.image.licenses="MIT" \
      org.opencontainers.image.version="${VLLM_VERSION}" \
      org.opencontainers.image.revision="${VLLM_GIT_SHA}" \
      org.opencontainers.image.created="${VLLM_BUILD_TIMESTAMP}"
