...
| :----------: | :--------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------: | :--------------------------------------------: |
| [Unreleased] |     -      | `cargo test --workspace --all-features` 1798 tests pass (post-P42 receiver-side closure; +4 `install_block_sink` + `engine.distributed_kv_cache` accessor) |             68.0% real (post-P42)              |
...
- **Multi-block KV-cache writes corrupted any prompt longer than one block (critical latent correctness bug)** — two KV-write paths disagreed with `read_kv` (the read path attention uses) about which block holds a token. `read_kv` iterates `block_ids` in block order (`block_ids[b]` holds tokens `[b·block_size, …)`), so the token at `global_pos` lives in `block_ids[global_pos / block_size]`. But `write_prefill_kv` indexed `block_ids[token_idx]` (treating it as one-per-**token**) and `read_decode_kv` used the raw block **index** as the block id. With `BLOCK_SIZE = 16` and a 20-token prefill into non-contiguous blocks `[5, 9]`, a `0..19` round-trip came back `[16,17,18,19,4,5,…,15,0,1,0,0]` — scrambled — and the decode token at position 20 landed in block 1 instead of block 9. Both only coincided with the correct layout when block ids are contiguous from 0 **and** the prompt fits one block, so every existing test (single block, `block_ids=[0]`) passed while **all real multi-block inference was corrupted**. Fix: both paths compute `block_idx = global_pos / block_size` and look up `block_ids[block_idx]`, matching `read_kv`. New `test_multi_block_kv_roundtrip_non_contiguous_blocks` (non-contiguous blocks, value-exact round-trip + decode placement) verified to fail pre-fix. (RIL ISS-019 / TASK-020.)
- **K8s health/readiness probes no longer require API-key auth (operational bug)** — the auth middleware was mounted app-wide with **no path exemption**, so when API keys were configured (the production case) the Helm chart's unauthenticated liveness/readiness probes (`GET /health/live`, `GET /health/ready`) received **401 and the pod never became ready** (restart loop). Verified empirically (with auth configured, `GET /health/live` without a key returned 401). Fix: a narrow `is_health_path` exemption passes `/health`, `/health/live`, `/health/ready`, `/ready` through without auth; `/metrics`, `/debug/*`, and `/shutdown` **stay authenticated** (the exemption is not a wholesale bypass — verified `/v1/models` still returns 401). The guard against an unauthenticated *public* deployment remains the startup refusal to bind non-loopback without keys. New tests `health_probes_are_exempt_from_api_key_auth` (failed pre-fix) and `non_health_endpoints_still_require_auth`. (RIL ISS-018 / TASK-019.)
- **Batched speculative draft generation advanced positions by `len()`, not `last_pos+1` (latent correctness bug)** — `generate_batched_drafts` advanced the per-sequence draft positions with `current_positions[i].push(current_positions[i].len())` — pushing the **count** instead of the next position. For a sequence decoding at position P the draft positions came out `[P, 1, 2, …]` instead of `[P, P+1, P+2, …]`, so the draft model applied RoPE at the wrong positions for every draft past the first, corrupting the proposed drafts. The newer per-seq path already advanced correctly (`last_pos + 1`); the legacy batched path (used when `draft_resolver` is `None`, i.e. `Engine::new_boxed` with a shared draft model) had diverged. Same bug class as the verification fix below, found by auditing the draft-generation side. Fix: advance with `last().copied().unwrap_or(0) + 1`. New `test_batched_draft_positions_advance_from_base` (position-recording draft model) verified to fail pre-fix (`got 1 in [2, 1]`). (RIL ISS-017 / TASK-018.)
- **Speculative-decoding verification used 0-based RoPE positions (latent correctness bug)** — `verify_draft_tokens_logits` built `verify_positions = [0, 1, …, n−1]` for the target-model verification forward pass, but RoPE consumes **absolute** positions (threaded unchanged through `forward_logits → forward_with_paged_kv → run_decoder_layers → LayerCtx.positions → attention`; `num_computed_tokens` is a separate field used only for KV-slot allocation, not a RoPE offset). So for any sequence past position 0 — i.e. **every speculative step after prefill** — the target model computed logits with RoPE at the wrong positions, corrupting the accept/reject decisions and the emitted tokens. The empty-drafts path already passed `batch.positions[i]` correctly; the drafts path was inconsistent with it. Fix: start `verify_positions` at the first input token's true position (`batch.positions[i][0]`). New `test_verification_uses_true_sequence_positions` (a position-recording mock) asserts every verification position is ≥ 2 after a 2-token prefill; verified to fail pre-fix (`got 0 in [0,1,2,3]`). The existing spec_dispatch tests missed this because their mock models ignore positions. (RIL ISS-016 / TASK-017.)
- **FP8 E4M3 KV-cache quantizer corrected (8x dequant error + underflow saturation)** — two bugs, both hidden by tests that never checked values. (1) `fp8_e4m3_to_float16` computed the exponent as `biased_exp − 7 − 3`, so **every dequantized value was 8× too small** (1.0 → 0.125, −3.0 → −0.375) — any model on an FP8 KV cache would have had all KV values scaled by ⅛, corrupting attention. (2) Encode underflow: values below the smallest normal (`|x| < 2⁻⁶`) produced a negative `biased_exp` whose `as u8` cast wrapped to a huge value and tripped the overflow check, so **tiny values mapped to the largest finite FP8** (0.001 → ~44). Fixes: decode exponent `= biased_exp − 7`; encode flushes underflow to zero, corrects the overflow threshold to `biased_exp > 14`, and handles the mantissa round-up carry. The round-trip test asserted **shape only** and the small-values test had a **vacuous** always-true clause; both now check actual values (round-trip within ~15% relative error, sub-normals flush to ~0) and were verified to fail pre-fix. FP8 KV cache is not yet wired into production (re-exported only), so no deployed impact, but the public primitive is now correct (worst normal-range round-trip error 4%). (RIL ISS-015 / TASK-015.)
- **RoPE rotation signs corrected to the standard convention (latent correctness bug)** — `apply_rope_with_inv_freq` computed `x1' = x1·cos + x2·sin`, `x2' = x2·cos − x1·sin`, which is the **inverse** rotation `R(−θ)`. The standard RoPE (Su et al. 2021) and HuggingFace `apply_rotary_pos_emb` (used by Llama / Qwen / Mistral) require `x1' = x1·cos − x2·sin`, `x2' = x2·cos + x1·sin`. Because both q and k pass through the same function, the relative-position term came out as `R(m−n)` instead of `R(n−m)` — the positional encoding's **direction was reversed for every RoPE model**. The cos/sin cache formula was already correct; only the combination signs were wrong. Latent for the same reason as the GQA bug: RoPE tests asserted only shape / determinism / positional-sensitivity / finiteness, never the rotation values against a reference, and there is no real-checkpoint validation (OPS-04 deferred). Fix flips the two sin-term signs; new `test_apply_rope_matches_standard_rotation_convention` pins the exact HuggingFace reference output and was verified to fail pre-fix (output confirmed to match the reference bit-for-bit after the fix). (RIL ISS-012 / TASK-012.)
- **MRoPE now rotates both halves of each section (latent correctness bug)** — `apply_rope_section` computed the first half correctly (`x1·cos − x2·sin`) but emitted the second half **unchanged**, applying only half the rotation and breaking the relative-position property for Qwen3.5-VL multimodal sections. Fix computes the second half (`x2·cos + x1·sin`); new `test_mrope_rotates_both_halves_standard_convention` (a single size-4 section ≡ standard RoPE) pins the reference output and was verified to fail pre-fix. (RIL ISS-013 / TASK-013.)
- **GQA attention now uses correct blocked head grouping (latent correctness bug)** — `expand_kv` (the shared helper behind `RopeGqaAttention`, i.e. the production attention path for Llama / Mistral / Qwen3) expanded KV heads with `Tensor::repeat`, which **tiles** (`[K0, K1, K0, K1, …]`). Grouped-query attention requires **blocked** (repeat-interleave) grouping `[K0, K0, …, K1, K1, …]` so query head `h` attends to KV head `⌊h / group_size⌋`; the tiled layout instead paired query head `h` with KV head `h % num_kv_heads` — the **wrong KV head for every group past the first**, silently corrupting attention output for all GQA models. The bug was latent because every `expand_kv` test asserted only the output *shape*, never the head *ordering*, and the engine has no real-checkpoint end-to-end validation (OPS-04 deferred). The correct blocked idiom already existed in `gemma4/attention/kernels.rs`; the shared util + flash paths had diverged. Fixed all three sites — `util::expand_kv` (`[batch,seq,heads,dim]`) and `flash_attention_v3` `GqaFlashAttention`/`MqaFlashAttention` `expand_kv` (`[batch,heads,seq,dim]`) — via the reshape→`broadcast_as`→contiguous→reshape repeat-interleave idiom, then `narrow` to the exact query-head count. 4 new regression tests, all verified to **fail on the pre-fix tiled code**: a direct ordering test (`[0,0,0,0,1,1,1,1]` not `[0,1,0,1,0,1,0,1]`), an end-to-end single-token `paged_attention` test (query heads {0,1} must output V0, {2,3} must output V1), and two flash-path ordering tests. (RIL ISS-010 / TASK-010.)
- **Gated DeltaNet `repeat_kv_heads` now uses blocked grouping (same bug class)** — a follow-up sweep of the ISS-010 tile-vs-blocked defect found a fourth site: the Gated DeltaNet (Mamba / SSM hybrid) `repeat_kv_heads` expanded K/Q heads to `num_v_heads` with `Tensor::repeat` (tiled). The DeltaNet recurrence is per-head, so V head `v` shares K/Q head `⌊v / group⌋`; the tiled layout paired V head `v` with the wrong K/Q head (`v % num_k_heads`). HuggingFace's canonical `repeat_kv` (used by Qwen3-Next `gated_delta_net`) is blocked. Fixed with the same repeat-interleave idiom; new `test_repeat_kv_heads_blocked_grouping` verified to fail pre-fix. (RIL ISS-011 / TASK-011.)
- **Seeded sampling now draws a per-step stream, not a frozen threshold** — `random_f32_seeded` re-seeded a fresh `StdRng` with the bare request seed on **every** decode step and read its first value, so a seeded generation drew the **same** random threshold at every step (per-step quantile lock): the nucleus / temperature cutoff froze and cross-step independence collapsed, producing degenerate repetitive output and contradicting OpenAI's seed semantic (reproducible *yet* per-step-varied). The per-step seed is now derived from `(seed, step)` via a fixed `SplitMix64` mix, where `step` is the already-generated seen-set length (deterministic for a given seed + prompt), so step `n` draws a distinct, reproducible position of the stream — same seed + same prompt ⇒ same output, distinct steps ⇒ independent draws. The unseeded thread-RNG path is unchanged. 6 new deterministic regression tests pin both halves of the contract; two pre-existing tests that asserted a fragile probabilistic "different seeds ⇒ different tokens" on a peaked distribution were hardened to assert the deterministic guarantee (distinct thresholds for distinct seeds). (RIL ISS-007 / TASK-007.)
- **`repeat_penalty` documentation corrected to per-distinct (not frequency-style)** — several doc comments falsely described `apply_repeat_penalty` as "frequency-style / proportional to occurrence count / divided once per occurrence (a token seen 3 times gets divided 3 times)". The implementation dedupes via a `HashSet` and applies the multiplicative, sign-aware adjustment **once per distinct token** (llama.cpp / HuggingFace repetition-penalty style). Corrected the `SamplingParams::presence_penalty` field doc, the server `frequency_penalty` honoring notes, and a test comment; explicitly noted that OpenAI's `frequency_penalty` is genuinely per-occurrence, so the engine's per-distinct `repeat_penalty` is an approximation (direction correct, count-scaling not). Docs-only, no behavior change. (RIL ISS-008 / TASK-008.)
- **Sampling no longer panics on empty logits rows** — the top-k mask blocks in `sample_batch` / `sample_one_with_params` and the `top_logprobs` ranking called `select_nth_unstable_by(n − 1)` on a potentially empty slice: a model returning an empty logits row (reaching the engine via `extract_per_seq_logits`) panicked the inference loop in both debug (subtraction overflow) and release (index out of bounds) — even though every other pipeline stage (greedy, temperature, top-p, argmax) degrades to greedy on empty input. The mask and ranking steps are now skipped when there is no distribution, restoring the module's defensive empty-input contract. 3 new unit tests (top-k + empty, top_logprobs + empty, batch row + empty).
- **`get_memory_pressure` no longer divides by zero on zero-block configs** — a degenerate config with zero KV blocks produced `NaN` pressure (0/0), poisoning scheduling decisions (preemption never triggered). Now reports maximum pressure (1.0) so the scheduler preempts; pinned by a new unit test. Also restored the clippy gate broken by the temperature-fix commit (a deny-tier `uninlined_format_args` error and `cast_lossless` warnings in the new statistical test).
- **Speculative KV rollback and preemption now honor the eviction refcount contract** — `MemoryManager::rollback` (and the no-caller legacy `MemoryManager::execute_preemption`) called `free`, which bypasses the eviction policy. Every block is `record_blocks`-ed on allocation and must be paired with exactly one `release_blocks` (ARCH-01); the stale refcount meant a re-allocated block's release decremented 2→1 instead of 1→0, so the allocator never got it back — a silent, accumulating KV-block leak under sustained speculative rejection — and a shared block could be freed underneath a prefix-cache co-owner. Both now release (decrement) and only return zero-refcount blocks to the allocator. New unit test asserts refcount 0 after rollback and that a re-allocated + released block returns to the pool.
- **Prefix-cache entries no longer pin decode blocks** — a finished sequence inserted its *entire* kv_blocks list (prompt + generated) into the radix-tree keyed by the prompt, so a hit returned more blocks than `matched_tokens` covers and one long response pinned every block it ever touched. Entries now hold only the prompt-covering blocks, keeping `matched_tokens` consistent with the returned block range. New engine test: a 16-token prompt with 48 generated tokens now caches 1 block instead of 4.
- **Stop-sequence finalization tolerates synthetic batches** — `finalize_stop_sequences` indexed `batch.sampling_params[i]` directly while the spec-verifier defensively uses `.get(i)`; the Batch docs sanction empty `sampling_params` ("equivalent to greedy decoding") on synthetic batches, so a seq/params length mismatch panicked the hot loop. Now degrades to "no stop sequences" on a missing entry. New malformed-batch regression test + positive-control test (stop-match still finishes the sequence before max_tokens).
- **`k8s/operator` removed** — the kubebuilder-scaffolded Go operator was dead code: it referenced the upstream `ghcr.io/vllm/vllm:latest` image (not vllm-lite), had no tests/CI/docs references, and vllm-lite's Kubernetes deployment is fully covered by the self-contained Helm chart + manifests. Removing it eliminates the 5 Go dependency alerts (golang.org/x/net, golang.org/x/oauth2) that were its sole source (~612 lines deleted). If operator-level automation is ever needed, it should be built as a vllm-lite-specific operator with tests and CI.
- **`husky-rs` restored as a dev-dependency with CI opt-out** — husky-rs (the author's own hook tool, [pplmx/husky-rs](https://github.com/pplmx/husky-rs)) is back as a server dev-dependency so dev machines with `prek` get automatic hook installation on `cargo build`/`cargo test`. Its build script's fail-fast behavior when `prek` is unavailable is its documented contract (hooks install automatically or the build fails; `NO_HUSKY_HOOKS` is the explicit opt-out) — verified against the README and confirmed as expected behavior, not a bug ([pplmx/husky-rs#20](https://github.com/pplmx/husky-rs/issues/20), closed as not-a-bug). Correct consumer integration: CI sets `NO_HUSKY_HOOKS=1` so the missing-prek path never triggers on GitHub runners.
- **CI repair round 4: checkpoint-fixture loader contract** — `crates/model/tests/support/on_disk.rs::loader()` panicked (via `require_weights` assert) when the checkpoint was absent, but the `fixture_loader_errors_without_weights` test (and its fallible-API contract) expects a typed `Err`. `loader()` now returns `candle` error "checkpoint not found ..." instead of panicking; the CI `Test` step and Matrix (stable/beta) jobs were failing on exactly this. With this fix, `cargo-deny`, `Benchmarks`, `MSRV Check`, and the main `CI` job are expected green; the remaining red items are the Go operator dependency alerts (5: `golang.org/x/net`/`x/oauth2` in `k8s/operator/go.mod`).
- **CI repair round 2** — first-round fixes took effect but exposed the next layer of pre-existing breakage: `candle-kernels`' build script requires `nvcc` (the CUDA toolkit), so the MSRV and ci-all-features jobs now install `nvidia-cuda-toolkit` (compiling CUDA kernels needs only the toolkit, no GPU); `cargo-deny` reported 5 workspace crates as unlicensed — they now inherit `license.workspace = true` from `[workspace.package]` (matching `vllm-dist`); and the fuzz jobs' `cargo +nightly fuzz` subcommand was not visible because `cargo-fuzz` must be installed into the nightly toolchain (`cargo +nightly install cargo-fuzz --locked`).
- **GitHub Actions CI repaired (5 root causes)** — CI was red on main for every workflow: (1) the unused `husky-rs` dev-dependency's build script fails whenever `.pre-commit-config.yaml` exists without the `prek` binary (its documented fail-fast contract; GitHub runners); removed from the workspace + server dev-deps (Cargo.lock cleaned) pending the NO_HUSKY_HOOKS opt-out approach. (2) `deny.toml` used a non-SPDX license name `Boost-1.0` — corrected to `BSL-1.0`, unblocking `cargo deny check`. (3) every `--all-features` build compiles `cudarc`, whose build script probes `nvcc --version`; GitHub runners have no CUDA toolkit, so `cudarc` panicked — CI workflows now set `CUDARC_CUDA_VERSION=12090` (cudarc's documented env override; verified locally with nvcc removed from PATH: check/build/doctest all pass). (4) the fuzz workflows called `just fuzz-build` and `cargo +nightly fuzz` without installing `just`/`cargo-fuzz` (`command not found`) — added `taiki-e/install-action` steps. (5) the mutation-nightly cargo-mutants cache step was missing `id: cargo-mutants-cache`, so the conditional install always ran and failed with "binary already exists" — added the id. Applies to `ci.yml`, `msrv.yml`, `fuzz.yml`, `fuzz-nightly.yml`, `mutation-nightly.yml`, `benchmark.yml`.
- **Mixtral MoE: invalid `top_k` now rejected with a typed error instead of panicking** — `MixtralSparseMoe` accepted any `top_k`; the routing `narrow(1, 0, top_k)` panicked when `top_k > num_experts`, and `top_k == 0` produced a zero weight-sum division (NaN routing). Both constructors now validate `1 <= top_k <= num_experts` (`validate_top_k`) and return a typed `candle` error naming `top_k`. 2 new rejection tests. (Expert selection itself is correct — candle's `arg_sort_last_dim(false)` sorts descending, verified against the crate source and the naive-vs-vectorized equivalence tests.)
- **Grafana dashboard aligned with the real `/metrics` metric names** — `docs/grafana/dashboard.json` queried `vllm_tokens_total`, `vllm_p50_latency_ms`, etc., names that only the *unrouted* `api::get_prometheus` produced; every panel therefore showed NoData while `/metrics` exposed the same values without the `vllm_` prefix. All 8 panel expressions now use the actual metric names (`tokens_total`, `latency_p50_ms`, `avg_latency_ms`, `requests_total`, `current_batch_size`, `avg_batch_size`). The dead 72-line `api::get_prometheus` (whose output existed nowhere on the wire and duplicated the exporter's data) is removed, along with its public-API baseline line; the metrics-timeout regression test now exercises the routed `/metrics` handler directly.
- **Prometheus `/metrics`: always-zero fake gauges removed** — `gpu_memory_used_bytes`, `gpu_memory_total_bytes`, `is_leader`, `inflight_requests`, and `scheduler_queue_size` printed literal `0` on every scrape (`EnhancedMetricsCollector::get_gauge` returns 0 for unsupported names; no backing data existed). Constant-zero metrics silently mislead dashboards and alerting, so they are removed; `inflight_requests` is superseded by the real `requests_in_flight` exported from the lock-free runtime snapshot (see the core-engine-metrics entry above).
- **Prometheus `/metrics` now exports core engine metrics** — the exporter previously covered CUDA-graph / speculative / queue gauges but omitted the core LLM-server counters: `tokens_total`, latency percentiles (`avg_latency_ms`, `latency_p50/p90/p99_ms`), `kv_cache_usage_percent`, `prefix_cache_hit_rate`, prefill/decode throughput, batch sizes, `requests_in_flight`, and scheduler wait time. The data was already collected in `EnhancedMetricsCollector`'s lock-free runtime snapshot; the function that exported it (`api::get_prometheus`) was never routed, so `/metrics` could not answer "how many tokens / what latency / how full is the KV cache". The exporter now reads the runtime snapshot directly (12 new metric families), avoiding the engine `GetMetrics` round-trip that can stall mid model-step. 2 new unit tests. Also documented (not yet changed) that `gpu_memory_*`, `is_leader`, `inflight_requests`, and `scheduler_queue_size` always print 0 — they have no backing data in the collector.
- **Prefix-cache entries no longer pin KV blocks forever** — finished sequences insert their KV blocks into the radix-tree prefix cache, and the cache holds a refcount on each block; there was no eviction path, so those blocks never returned to the allocator. A long-running server exhausted the block pool even with zero running sequences (verified: a 16-block pool stayed at 16/16 used after 16 distinct finished prompts), and new requests then ran KV-less. Fix: `execute_preemption` now clears the prefix cache (via new `RadixTree::drain_blocks`, which collects every entry's block list and resets the tree) before evicting live sequences, releasing the cache's refcounts so the allocator can reuse the blocks; sequences still running on a shared prefix keep their own refs and are unaffected. New `drain_blocks` unit tests + `prefix_cache_eviction` integration test (16-block pool: 16 finished prompts, then a fresh-prompt request completes with `used < total`).
- **cargo-fuzz tooling restored** — `fuzz/Cargo.toml` declared `[lints] workspace = true`, but `fuzz` is excluded from the root workspace (`exclude = ["fuzz"]`), so cargo errored with "failed to find a workspace root" and **every** `cargo +nightly fuzz` invocation — `just fuzz-build`, `just fuzz-smoke`, and the fuzz-nightly CI job — failed at manifest-parse time before a single fuzz case ran. Fuzz targets are libFuzzer harnesses built only via cargo-fuzz and don't need the workspace's clippy/rustc lint tiers, so the invalid inheritance was removed (with a comment explaining why). Verified: `cargo +nightly fuzz build` green; 6 targets fuzzed clean (app_config_yaml 1.0M, openai_http_request 12.2M, qwen3_config_json 14.8M, batch_json_input 12.0M, completions_http_request 12.0M, tokenizer_decode 6.0M executions — ~58M total, zero crashes/panics/timeouts).
- **Prefix cache: radix-tree `matched_tokens` overcounted past an incomplete node** — `RadixTree::longest_prefix_match` updated `matched_len` on *every* matched child but `matched_blocks` only on *complete* nodes. With complete entries `[1,2]` and `[1,2,3,4]` (node 3 left incomplete as an intermediate), a query for `[1,2,3]` reported `matched_tokens = 3` while returning the 2-token blocks — the scheduler's `resolve_prefix_tokens` treats `matched_tokens` as the KV-computed token count, so it would skip 3 tokens with only 2 tokens' worth of KV blocks, corrupting reuse. Fix: `matched_len` now updates only when a complete node is hit, keeping it in lockstep with the returned blocks. Regression test `test_radix_tree_matched_tokens_tracks_complete_prefix` (failed pre-fix: 3 vs 2). Exact-match and longest-prefix-bounded property tests still pass.
- **Speculative decoding: prefill-step verification used wrong logits offsets** — `verify_draft_tokens_logits` compared draft token `j` against the target model's logits at offset `j × vocab` (verify-sequence position `j`) instead of `(input_len − 1 + j) × vocab` (the position whose logits actually predict the draft). The math is exact for decode batches (`input_len == 1`, the common path), but for **prefill batches** — the first speculative step of a new request, where the input is the full multi-token prompt — draft tokens were compared against the target's predictions for *input* tokens: correct drafts got rejected (and, in mixed batches, wrong tokens could be accepted), silently changing the output distribution. The bonus-token offset (`accepted × vocab`) had the same off-by-(input_len−1). Fix: offsets now start at `input_len − 1`, with a regression test (`verifier_prefill_accepts_drafts_at_position_after_prompt`) using a next-token-predicting model that failed pre-fix (`accepted_counts = [0]` instead of `[2]`) and passes post-fix. Decode-path behavior unchanged (same offsets for `input_len == 1`).
- **Ops endpoints no longer hang during long model steps** — `/health/details` and `/debug/kv-cache` waited on `response_rx.recv().await` with no timeout for the engine's `GetMetrics` reply. The engine only answers between synchronous model steps, which can take seconds-to-minutes on a large batch — so a Prometheus/ops scrape issued mid-step blocked the HTTP request for the entire step. All three engine-metrics waits (`health_details`, `kv_cache_dump`, and the unrouted `api::get_prometheus`) now use a bounded 2 s wait (`METRICS_RESPONSE_TIMEOUT`) and fall back to defaults, matching the endpoints' documented "never block the caller" intent (previously the fallback only fired on channel close, not on a live-but-busy engine). 3 regression tests in `crates/server/tests/metrics_timeout.rs` exercise the timeout path with a live-but-unread engine mailbox (each test completes in exactly 2.00 s, proving the bounded wait fires). Zero public-API delta (the const is `pub(crate)`).
- **Embeddings requests now charged for input length by the cost-aware rate limiter** — `estimate_request_cost` only counted `messages` (chat) and `prompt` (completions); an embeddings body (`{"input": [...]}`) fell through to the flat `1.0` default, so a 100k-word input cost the same as a 1-word input on a compute-proportional endpoint. With API keys enabled this let a caller under-pay the rate-limit budget while pinning the encoder with arbitrarily long inputs. Fix: the estimator now charges the `input` length — string or array of strings → summed whitespace word counts, non-string arrays → element count, other shapes → `1.0`, clamped to `[1, 100_000]`. Embeddings have **no generation budget**, so the `max_tokens` addend is deliberately NOT applied (an earlier draft added the default 100 and over-counted every embeddings request, which would have starved legitimate users of the endpoint). 5 new unit tests cover array-of-strings, single-string, a 10k-word regression (was `1.0`, now `10_000.0`), token-array element counting, and the empty-input `1.0` floor; plus 1 new integration test in `rate_limit_headers.rs` (capacity-10 bucket: two 4-word + one 2-word request succeed, the next is 429). `estimate_request_cost` is `pub(crate)`; no public-API delta.
- **HA concurrency hygiene** — two small lock-handling fixes in stability-critical components: (1) `CircuitBreaker::check_and_transition` no longer holds the `state` write lock while acquiring `last_failure_time.read()` — the nested acquisition created an inconsistent lock order vs `on_failure` (which writes `last_failure_time` before `state`); the recovery-timeout check now reads the timestamp first and re-checks the `Open` state under the write lock, eliminating the latent ABBA hazard. (2) `FailoverManager::on_node_failure` used a write lock for a read-only scan (collecting seq_ids to migrate); it now takes a read lock so concurrent `track`/`complete`/`migrate` writers are not blocked exactly when a node failure triggers mass migration. Existing unit tests cover both (16 circuit-breaker + 34 failover tests); behavior unchanged, no API delta.
- **All-`-inf` / NaN logits no longer poison temperature and nucleus sampling** (v31.0 "zero known correctness bugs" gate) — `temperature_sample` and `top_p_sample` computed `exp(v - max)` over degenerate rows (every logit `-inf`, or a `NaN` logit), producing `NaN` probabilities whose cumulative sum never crosses the random threshold, so the samplers fell through to returning the **last vocabulary index** from `NaN` math. The sibling logprob helpers (`log_softmax`, `renormalized_top_p_logprobs`) already guarded `!max_val.is_finite()`; the samplers were inconsistent with them. Fix: both samplers now fall back to `greedy_sample` (deterministic argmax — first index for all-equal `-inf`) when the max logit is non-finite, and the `top_k` masking path inherits the same safety. 4 new unit tests cover temperature/top-p/top-k all-`-inf` + `NaN` rows and the public `sample_batch_with_params` surface (degenerate rows yield the first token with `-inf` logprob). No behavior change for any finite-logit distribution; only degenerate model output changes from arbitrary-tail-index to deterministic-first-index.
- **Production middleware stack ordering fixed** — `main.rs` mounted the security middleware in the reverse of the documented order (axum's `Router::layer` wraps with the *last-applied* layer outermost). With API keys enabled, `auth_middleware` ran **outside** the body-size limit, so it read (and, past its 1 MiB `to_bytes` cap, silently truncated to empty) oversized request bodies *before* the size-limit layer could reject them: a 2 MiB body reached handlers as an empty body and came back as 400/200 instead of 413, and 401/429 responses from auth bypassed the correlation middleware entirely (no `X-Request-ID`). Fix: extracted the production router into `vllm_server::app::build_app` with the correct stack — `cors` (outermost) → `correlation_id` → `audit` → `body_size_limit` → `auth` → handlers — matching the ordering documented in `security::audit_middleware`; and `auth_middleware` now surfaces a 413 (instead of truncating) when the size-limit-bounded body stream errors mid-read (memory stays bounded by the always-mounted limit layer). 6 new integration tests in `crates/server/tests/middleware_wiring.rs` pin the contract: oversized body → 413 (not truncated), 413/401 both carry `X-Request-ID`, valid under-limit requests pass auth+limit, invalid keys → 401. No public API delta beyond the new `app` module; zero behavior change for deployments without API keys.
- **Deprecated public shims removed** (v31.0 31-C "Migrate deprecated import paths" closure) — the last three deprecated legacy surfaces had zero internal callers and their deprecation windows (documented as "one minor release" / "before 1.0") had elapsed: `vllm_core::speculative::draft_registry` re-export module (orphan file — not even compiled since the `registry/` sub-tree reorganization), `vllm_dist::NcclAllReduce` alias (misleading NCCL name; `LocalSumAllReduce` is the honest name) including its `dist` + `tensor_parallel` re-exports, and `vllm_server::openai::types::EmbeddingData` alias (`Embedding` is the canonical type). Internal callers were already migrated in earlier batches, so this is removal-only: no code call sites changed, no behavior delta. Public-API baselines refreshed via `just public-api-baseline` (API shrinks are always allowed by the Phase 12e gate); the refresh also caught up pre-existing stale entries (recent `PartialEq`/`Eq` derives, `AuthMiddleware::new_with_overrides`, `AuthConfig::rate_limit_overrides`, `with_cors` signature, etc.) so the committed baselines now match the current toolchain output exactly.
- **All-targets clippy gate restored** — `cargo clippy --all-targets --workspace --all-features -- -D warnings` (the AGENTS.md pre-commit gate) previously failed with 13 errors across 6 files. Fixed: `clippy::float_cmp` on exact-literal f32 assertions in `vllm-traits/src/sampling.rs` and `vllm-core/src/types/request.rs` tests (allowed with rationale — literals are exactly representable), `unsafe_code` in the `sequence_packing.rs` env-var test module (allowed with per-block SAFETY rationale), items-after-statement in `multi_gpu_scheduler.rs` (moved `use` to function top), `cast_lossless` + `collection_is_never_read` in `gpu_engine_e2e.rs` (`u64::from(i)`; receivers kept alive with allow + rationale), `future_not_send` in the multi-node gRPC bootstrap (documented: only call site awaits directly on the main task), and `significant_drop_tightening` in `paged_kv_cache_wrapper.rs` (guard now acquired per iteration — `read_layer_block` returns owned `Vec`s). The documented gate is now green.
- **`// invariant:` comments added to all production `unwrap()`/`expect()` sites missing them** (AGENTS.md invariant policy — "treat it as a bug"). 16 sites across 7 files: `rope.rs` (5× `dims4().expect("dims4")` — RoPE callers pass rank-4 query tensors by construction), `auth.rs` (2× response-builder empty-body unwraps), `health_handlers.rs` (RwLock read), `openai/chat.rs` (4× serde serialize of plain derived structs + 1× `seq_id_rx` construction invariant), `openai/completions.rs` (2× `seq_id_rx`), `paged_kv_cache_wrapper.rs` (`Arc::try_unwrap` sole-ownership contract), `testing/device.rs` (test-only CUDA helper). No behavior change; zero test delta.
- **Rate-limit cost estimation now accounts for `n` and `best_of` parameters** — the token-bucket rate limiter's `estimate_request_cost` helper computed request cost as `prompt_tokens + max_tokens` only, ignoring the `n` (number of completions) and `best_of` parameters. A single request like `{"prompt": "hi", "max_tokens": 100, "best_of": 100}` had the same rate-limit cost as `best_of=1` but generates 100x the output tokens — a rate-limit bypass that let a caller pin the scheduler with sub-budgeted cost. Fix: the cost now multiplies by `max(n, best_of)` (clamped to `[1, 100_000]`), with `n <= 0` / `best_of <= 0` treated as 1. `max(n, best_of)` is the correct multiplier because the completions validator rejects `n > 1 && best_of > 1` (they can't both exceed 1), so exactly one of them is the active compute multiplier. 16 new unit tests cover: `n` multiplication, default `n=1` / `best_of=1`, `n=0` / negative treated as 1, `best_of` multiplication, `max(n, best_of)` selection, out-of-range clamping, and chat-vs-completions parity. `estimate_request_cost` is `pub(crate)` so there is no public-API delta; this is a security/correctness hardening of the auth middleware.
...
- **Stop-sequence-matched sequences now marked Finished in the scheduler** (correctness bug) — when `Engine::step_regular` detected a `stop_token_sequences` match, it called `finalize_finished(seq_id, FinishReason::Stop)` which only dropped the response channel and sent the finish reason to the HTTP handler. The sequence was **not** marked `Finished` in `SchedulerEngine`, so it stayed in `running` with `Status::Decoding` and was re-included in every subsequent `build_batch`. The model kept generating tokens that were silently dropped (response channel already removed), wasting compute and leaking KV cache blocks until `max_tokens` was eventually reached. **Fix**: new `SchedulerEngine::finish_sequence(seq_id)` method (mirrors `update`'s finished-sequence handling — marks status, dispatches `SequenceFinished` observer event, releases KV blocks, moves to `self.finished`) is now called for each newly-stopped sequence before `finalize_finished`. This excludes the sequence from future batches and releases its blocks immediately. The server-side stop tests previously used a mock engine that simulates `finalize_finished` directly, so they did not catch this. New regression test `test_finish_sequence_excludes_from_future_batches` in `crates/core/tests/scheduler_integration.rs` exercises `SchedulerEngine::finish_sequence` with a real scheduler: after the call, `running_count` drops to 0, `get_sequence` returns `None`, `build_batch` returns an empty batch, and the sequence appears in `finished_sequences`. Public-API delta: 1 new method (`SchedulerEngine::finish_sequence`); CHANGELOG `[Unreleased]` already has a `public-api: vllm-core` bullet per the public-api-check gate.
- **`SlowModel::forward_logits` returned wrong tensor shape** (testing infrastructure) — the method produced 1 float per input token (matching `embed`'s shape) instead of `vocab_size` floats per token. All other mock models (`StubModel`, `IncrementModel`, `ConstModel`, `FakeModel`, `NeverProgressModel`) return `tokens.len() * vocab_size` floats per sequence per the `ModelBackend::forward_logits` contract. If the engine's `forward_logits` path (the production path, not the legacy `forward`) was used with a `SlowModel`, the mismatched shape would cause a panic in the sampler's logit slicing. Fix: `forward_logits` now returns `vec![0.0f32; tokens.len() * vocab_size]` per sequence (all-zero logits, no peak — consistent with `SlowModel`'s role as a timing-only mock). Dormant bug — `SlowModel` is only re-exported via `vllm_testing::SlowModel` and no existing test exercises its `forward_logits` path; the fix was discovered while adding trait-method test coverage (24 new unit tests across 5 mock models + `SlowModel`). No public-API delta (method signature unchanged; only the returned tensor shape is corrected).
- **`forward_logits` + `embed` + metadata trait methods on all test mock models** (testing infrastructure coverage) — added 24 unit tests covering `forward_logits`, `embed`, `vocab_size`, `num_layers`, and `num_heads` across `StubModel`, `IncrementModel`, `ConstModel`, `FakeModel`, and `NeverProgressModel` (the 5 mock `ModelBackend` implementations in `crates/testing/src/mocks/mod.rs`). These methods are called by engine integration tests but had **zero** test coverage (only `forward()` was tested). `mocks/mod.rs` line coverage went **70.11% → 100.00%**, function coverage **53.57% → 100.00%**. Additional tests for `SlowModel` (delay, builder methods, `forward_logits`, `embed`, trait metadata) raised `slow_model.rs` line coverage from **60.58% → 97.68%** and the `vllm-testing` crate total from **~77% → 93.41%**. No behavior change; tests pin existing implementations and the one bug fix above.
- **Clippy pedantic lint cleanup** — resolved 8 pedantic warnings (redundant closures, `&Option<T>` → `Option<&T>`, `i32 as u8` sign-loss, variable shadowing, `&mut self` → `&self`, wildcard match → explicit variant) across 4 files in `vllm-core`, `vllm-server`, and `vllm-dist`. All fixes are type-level only: no behavior change, no public-API delta, no test delta. `just ci` remains green (1798+ tests, 80.46% doc-coverage). **4 files, minimal diffs.**
- **Fuzz CI matrix expanded to all 8 targets** — `.github/workflows/fuzz.yml` (PR smoke) and `.github/workflows/fuzz-nightly.yml` (nightly long-run) only covered 3 of 8 fuzz targets (`app_config_yaml`, `qwen3_config_json`, `safetensors_header`), leaving 5 security-sensitive targets untested in CI: `batch_json_input` (batch API request parsing), `completions_http_request` (legacy completions endpoint), `gguf_header` (GGUF checkpoint header), `openai_http_request` (chat request parsing), `tokenizer_decode` (tokenizer decode). These targets fuzz deserialization of untrusted network input and model file headers — exactly the attack surface fuzzing is designed to catch. Both matrices now include all 8 targets. `docs/fuzz.md` target table + CI integration section updated to match. CI-only change — no Rust code touched.
- **OTLP Metrics + Traces Exporter (P43 T1-T6)** — opt-in push-based OpenTelemetry exporter behind a new `opentelemetry` Cargo feature on both `vllm-core` and `vllm-server`. Off by default; `cargo build` / `cargo test` without `--features opentelemetry` is unaffected.
    - **T2**: `OtlpConfig` + `OtlpProtocol` + `OtlpError` types with `validate()` covering `trace_sampling_ratio ∈ [0,1]`, non-zero `metrics_export_interval_secs`, non-empty `endpoint`. `#[serde(default)]` per-field so partial YAML configs work.
    - **T3**: `OtlpExporter` background tokio task + 21-entry `SCHEMA_MAP` translating Prometheus metric names → OTel instrument names. Polls `EnhancedMetricsCollector` every `metrics_export_interval_secs`; ships via `opentelemetry-otlp` `grpc-tonic`. `PeriodicReader` auto-flushes per tick.
    - **T4**: `tracing_init.rs` — `init_tracing_with_otlp()` wraps the `tracing_subscriber` registry with a `tracing-opentelemetry` layer bridging every `tracing::info_span!` → OTLP span. `OtlpGuard` (Drop-flush) shuts down the `SdkTracerProvider` on graceful exit. `const fn` constructors + `#[must_use]`.
    - **T5**: Server bootstrap — `ObservabilityConfig` in YAML config (`observability.otlp.*` section) + `--otlp-endpoint` CLI flag (env: `VLLM_OTLP_ENDPOINT`). `spawn_otlp_exporter()` takes the engine's `Arc<EnhancedMetricsCollector>` and returns an `OtlpHandle` (Drop-flush: aborts task + `exporter.shutdown()`). `init_tracing_with_otlp` replaces `logging::init_logging` when OTLP is enabled.
    - **T6**: Integration tests with in-process stub OTLP collector (`otlp_stub_collector.rs` — implements gRPC `MetricsService` + `TraceService`) + 5 tests in `otlp_exporter_integration.rs` (metrics arrive at stub, unreachable endpoint doesn't crash, disabled default, config validation rejects bad sampling ratio, traces arrive). `opentelemetry-proto` + `tonic` added as dev-dependencies.
    - **New deps**: `opentelemetry`, `opentelemetry_sdk`, `opentelemetry-otlp`, `opentelemetry-semantic-conventions`, `tracing-opentelemetry` (on `vllm-core`); `opentelemetry-proto`, `tonic` (dev-deps on `vllm-server`).
    - **public-api: vllm-core added** — `OtlpConfig`, `OtlpProtocol`, `OtlpError`, `OtlpExporter`, `OtlpExporterBuilder`, `tracing_init::init_tracing_with_otlp`, `tracing_init::OtlpGuard`.
    - **public-api: vllm-server added** — `config::ObservabilityConfig`, `config::AppConfig.observability`, `cli::CliArgs.otlp_endpoint`, `bootstrap::observability::spawn_otlp_exporter`, `bootstrap::observability::OtlpHandle`.
    - **CI**: new `ci-otlp` job in `.github/workflows/ci.yml`. **ADR**: `docs/adr/ADR-021-otlp-exporter.md`. **Docs**: `OPERATIONS.md` §"OTLP Observability" with Jaeger/Tempo/Datadog/Honeycomb examples + troubleshooting table. **Feature matrix**: `docs/reference/feature-matrix.md` updated with `opentelemetry` rows.
    - **Test count delta**: +4 config tests + 4 CLI tests + 5 integration tests (13 new tests total across T2-T6).
    - Closes the "OTLP" v32+ code follow-up listed in the v31.0 production-readiness doc.
...
- **Receiver-side multi-node KV block transfer closure (v31.0 P42 T1–T5)** — completes the vLLM-equivalent "fetch from peer, install locally, hit locally next time" loop that the protocol spec (P14) deferred to the receiver-side wiring phase. P42 layers five commits: **T1** `PagedKvCache::write_block_bytes` pub helper that installs one block's wire-shape bytes across all layers (mirrors T40's `read_layer_block` / `fetch_block`); **T2** new `distributed_kv::block_sink::{BlockSink, WriteError, MockBlockSink}` — the receiver-side trait counterpart to `BlockDataSource`, with a one-line `MockBlockSink` for tests; **T3** `PagedKvCacheWrapper` impls both `BlockDataSource` (sender) and `BlockSink` (receiver) so one production wrapper can serve both roles; **T4** `DistributedKVCache::with_block_sink` (builder) + auto-install in `maybe_install` after a successful peer fetch — gated by the existing `install_on_fetch` toggle (defaults to `true`); **T5** server bootstrap wires the engine's `PagedKvCacheWrapper` as the cache's `BlockSink` so every `fetch_block` from a peer installs into the local cache. The T5 plumbing required: `engine.paged_kv_cache` field retyped from `Option<Arc<PagedKvCache>>` to `Option<Arc<parking_lot::Mutex<PagedKvCache>>>` so the engine and the wrapper share the same underlying data; `set_paged_kv_cache` wraps the incoming cache in the Mutex (requires unique Arc ownership — the builder hands the cache in by value); new `engine.distributed_kv_cache()` accessor for bootstrap-time `install_block_sink` wiring; new `PagedKvCacheWrapper::from_arc_mutex` constructor so the bootstrap can build a sink-side wrapper sharing the same `Arc<Mutex<PagedKvCache>>` with the engine; `DistributedKVCache::install_block_sink` post-construction setter (the cache is held by `Arc<DistributedKVCache>` on the bootstrap side so a builder setter would be unusable — the field is now `parking_lot::Mutex<Option<Arc<dyn BlockSink>>>` to enable late binding); `parking_lot` promoted from transitive to direct dep on `vllm-core` / `vllm-dist` (gated by `--features multi-node`). **4 new tests** (`install_block_sink_wires_sink_post_construction`, `install_block_sink_replaces_previously_wired_sink`, `engine_distributed_kv_cache_accessor_returns_some_when_wired`, `engine_without_distributed_kv_returns_none_for_cache_accessor`). **Public-API delta (default features): 0.** Public-API delta (`--features multi-node`): +1 `pub` method on `Engine` (`distributed_kv_cache`), +1 `pub` constructor on `PagedKvCacheWrapper` (`from_arc_mutex`), +1 `pub` method on `DistributedKVCache` (`install_block_sink`). Closes the Phase 31-G "receiver-side `write_kv_batch` deferred to P42+" item from the v31.0 master plan. ADR-020 status line bumped from "receiver-side write_kv_batch deferred to P42+" to "receiver-side BlockSink shipped in P42; 31-G complete".

### Changed

- **public-api: vllm-model changed** — `Architecture::create_model` now returns `(Box<dyn ModelBackend>, Option<Arc<Mutex<PagedKvCache>>>)` so the paged KV cache is extractable for multi-node block transfer; `CausalLm` / `HybridLm` / `Qwen35HybridModel` store `kv_cache: Arc<Mutex<PagedKvCache>>` (was bare `PagedKvCache`) and expose `paged_kv_cache()`; `ModelLoader::paged_kv_cache_clone()` now returns the captured cache (was an unimplemented `None` TODO). `parking_lot` promoted from optional to always-on in `vllm-model`. Fixes the multi-node engine-bootstrap compile error (`server/src/bootstrap/engine.rs` `with_paged_kv_cache` type mismatch).

- **public-api: vllm-core changed** — `EngineBuilder::with_paged_kv_cache` and `Engine::set_paged_kv_cache` now accept `Arc<Mutex<PagedKvCache>>` directly (the shape `ModelLoader::paged_kv_cache_clone` yields) instead of `Arc<PagedKvCache>`, replacing the internal `Arc::try_unwrap` + re-wrap. (`--features multi-node` only; default-features delta: 0.)
...
- **public-api: vllm-testing added** — 7 new items for the GPU-first device helpers (GPU testing infrastructure): new `pub mod device` module with 3 public helpers (`cuda_available() -> bool`, `gpu_device() -> Device`, `gpu_or_cpu() -> Device` implementing the policy "default to CUDA when available, fall back to CPU") plus 3 re-exports at the crate root (`vllm_testing::cuda_available`, `vllm_testing::gpu_device`, `vllm_testing::gpu_or_cpu`) plus the `pub mod device` re-export itself. These helpers enable model tests to use `gpu_or_cpu()` instead of hard-coding `Device::Cpu`, so the same test runs on CUDA when a GPU is present and falls back to CPU otherwise (GPU-first testing policy). No behaviour change for CPU-only CI runners — `gpu_or_cpu()` returns `Device::Cpu` when CUDA is unavailable, preserving backward compatibility with the existing test suite.
