...
- **`husky-rs` restored as a dev-dependency with CI opt-out** — husky-rs (the author's own hook tool, [pplmx/husky-rs](https://github.com/pplmx/husky-rs)) is back as a server dev-dependency so dev machines with `prek` get automatic hook installation on `cargo build`/`cargo test`. Its 0.4.0 build script hard-fails the build when a prek config exists but `prek` is missing (or any `prek` command fails) — a minimal reproduction was written and filed upstream ([pplmx/husky-rs#20](https://github.com/pplmx/husky-rs/issues/20)). All CI workflows set `NO_HUSKY_HOOKS=1` so the missing-prek path never triggers on GitHub runners.
