...
- **Clippy pedantic lint cleanup** — resolved 4 pedantic warnings (redundant closures, `&Option<T>` → `Option<&T>`, `i32 as u8` sign-loss, variable shadowing) across 4 files in `vllm-core`, `vllm-server`, and `vllm-dist`. All fixes are type-level only: no behavior change, no public-API delta, no test delta. `just ci` remains green. **4 files, minimal diffs.**
