...
- **Embeddings requests now charged for input length by the cost-aware rate limiter** — `estimate_request_cost` only counted `messages` (chat) and `prompt` (completions); an embeddings body (`{"input": [...]}`) fell through to the flat `1.0` default, so a 100k-word input cost the same as a 1-word input on a compute-proportional endpoint. With API keys enabled this let a caller under-pay the rate-limit budget while pinning the encoder with arbitrarily long inputs. Fix: the estimator now treats `input` like a prompt — string or array of strings → summed whitespace word counts (with the `max_tokens` default applied, consistent with the other endpoints), non-string arrays → element count, other shapes → `1.0`. 4 new unit tests cover array-of-strings, single-string, a 10k-word regression (was `1.0`, now `10_100.0`), and token-array element counting. `estimate_request_cost` is `pub(crate)`; no public-API delta.
