...
impl TensorParallelConfig {
    /// Construct a config, returning `None` if `world_size` is zero, `rank`
    /// is out of range, or `device_ids` length doesn't match `world_size`.
    #[must_use]
    ...
    /// Resolve the GPU device for this rank, falling back to CPU if CUDA is unavailable.
    #[must_use]
