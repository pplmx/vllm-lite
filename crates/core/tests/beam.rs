...
fn test_beam_sequence_push() {
    ...
    beam.push(3, 0.3);
    assert_eq!(beam.tokens, vec![1, 2, 3]);
    assert!((beam.score - 0.8).abs() < 1e-6); // 0.5 + 0.3
}
...
fn test_beam_width_one_equals_greedy() {
    ...
    // invariant: beam tokens are bounded by max_tokens; f32 precision loss is
    // acceptable for the length-penalty normalization in this test.
